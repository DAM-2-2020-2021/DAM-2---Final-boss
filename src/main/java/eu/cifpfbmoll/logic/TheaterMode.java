...
public class TheaterMode extends JFrame implements Runnable {
    ...
    public int getMyAdminIs() {
        return myAdminIs;
    }

    public void setMyAdminIs(int myAdminIs) {
        this.myAdminIs = myAdminIs;
    }
    ...
    public TheaterMode() {
        ...
        this.sound = Sound.clipSoundMenu(20);
        this.sound.start();
        Assets.init();
        ...
        nodeManager.register(Message.class, (id, message) -> {
            switch (message.getMessageType()) {
                case NICKNAME:
                    ...
                    for (int i = 0; i < this.allSpacecrafts.size(); i++) {
                        if (this.allSpacecrafts.get(i).getID() == id) {
                            shipExists = true;
                        }
                    }
                    if (!shipExists) {
                    ...
                    System.out.println(this.allSpacecrafts.size());
                    break;
                case TEAM:
                ...
                case SPACECRAFT_TYPE:
                    ...
                    break;
                case DISCOVER:
                    System.out.println("discover recived");
                ...
                case ADMIN:
                    if (!imAdmin){
                        ...
                        if(message1 == null){
                            System.out.println("mensaje es nulo");
                        }
       ...
       /* for (int i = 0; i < 15; i++) {
        ...
        this.pcList.put(23,"192.168.0.23");
    ...
    public void startGame() {
        ...
        theaterActive = false;
        GameMode gameMode = new GameMode(nodeManager, redTeam, blueTeam, configuration,this);
        sound.stop();
    ...
    public void run() {
        ...
        while (theaterActive) {
            try {
                System.out.println("im adming "+ imAdmin);
                if (imAdmin) {
                    //Todo los nodos no se actualizar
